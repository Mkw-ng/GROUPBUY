...
/**
 * Check if Power Drop has expired (activatedAt + 48 hours < now).
...
export async function getActiveOrderCounts(): Promise<{
  ...
  const db = await getDb();
  if (!db) return { all: 0, pending: 0, invoice_issued: 0, paid: 0, pickup_available: 0, cancelled: 0 };
  const rows = await db
...
export async function updateOrderStatus(id: number, status: "pending" | "invoice_issued" | "paid" | "pickup_available" | "cancelled"): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function checkAndExpirePowerDrop(): Promise<boolean> {
  ...
  const deadline = activated + 48 * 60 * 60 * 1000; // 48 hours
